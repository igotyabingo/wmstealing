...
class WatermarkBase:
    def __init__(
        ...
        gamma: float = 0.5,
        delta: float = 2.0,
        seeding_scheme: str = "lefthash",  # simple default, find more schemes in alternative_prf_schemes.py
...
class WatermarkDetector(WatermarkBase):
    ...
    * z_threshold -> Changing this threshold will change the sensitivity of the detector.
    ...
    def _compute_p_value(self, z): ##### required
    ...
    def _score_sequence(  # noqa: C901
        ...
        if return_p_value:
            ...
            score_dict.update(dict(p_value=self._compute_p_value(z_score))) # 1. z_score 계산 -> p_value 계산
        if return_token_mask:
    ...
    def detect(  # noqa: C901
        ...
        **kwargs,
        ...
        if window_size is not None:
            ...
            score_dict = self._score_sequence(tokenized_text, **kwargs) # score_dict[']
        if return_scores:
            output_dict.update(score_dict)
        # if passed return_prediction then perform the hypothesis test and return the outcome
        if return_prediction:
        ...
        # output_dict에 raw_prompt, generated_text, p_value만 포함되면 됨: detect method에서 p_value만 새로 계산
        return score_dict["p_value"]
