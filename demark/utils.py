...
class QueryDataset(Dataset):
    ...
    I need you to randomly choose a phrase without exact meaning. Randomly start your answer with: \"'''
            ...
            t1_ids = self.tokenizer(t1, add_special_tokens=False, return_tensors="pt").input_ids[0].cuda()
            t2_ids = self.tokenizer(t2, add_special_tokens=False, return_tensors="pt").input_ids[0].cuda()
            t3_ids = self.tokenizer(t3, add_special_tokens=False, return_tensors="pt").input_ids[0].cuda()
        ...
        elif model_id in ["Qwen/Qwen3-4B-Instruct-2507", "Qwen/Qwen2.5-7B-Instruct"]:
            t1 = "<s>[Round 1]\nUser: I need you to randomly choose a phrase without exact meaning. Randomly start your answer with: \""
            t2 = '" or "'
            t3 = "\"\nAssistant: "
            t1_ids = self.tokenizer(t1, add_special_tokens=False, return_tensors="pt").input_ids[0].cuda()
            t2_ids = self.tokenizer(t2, add_special_tokens=False, return_tensors="pt").input_ids[0].cuda()
...
def generate_random_choose_probs(
    ...
    for token_list_indices, inputs in DataLoader(
        ...
        if not blackbox:
            ...
            probs_res_matrix[token_list_indices[:, 0], token_list_indices[:, 1]] = s1.to(probs_res_matrix.dtype)

            probs_res_matrix[token_list_indices[:, 1], token_list_indices[:, 0]] = -s1.to(probs_res_matrix.dtype)
